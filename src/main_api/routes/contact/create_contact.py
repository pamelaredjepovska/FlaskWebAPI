...
def create_contact():
    try:
        ...
        insert_query = """INSERT INTO contact (name, country_id, company_id) VALUES (%(name)s, (SELECT id FROM country WHERE name = %(country_name)s), (SELECT id FROM company WHERE name = %(company_name)s)) RETURNING *"""
        insert_query_params = {
            "name": contact_data.name,
            "country_name": contact_data.country,
        ...
        return {"message": "Contact record inserted successfully."}
...
class ContactInput(BaseModel):
    ...
    country: constr(strip_whitespace=True, min_length=1)  # type: ignore
    company: constr(strip_whitespace=True, min_length=1)  # type: ignore
